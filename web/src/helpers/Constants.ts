export const Constants = Object.freeze({
  PRESTON_WERNER_VENTURES: 'https://prestonwernerventures.com/',
  README: 'https://github.com/redwoodjs/redwood/blob/main/README.md',
  SHOP: 'https://shop.redwoodjs.com/',
  COMMUNITY:
    'https://community.redwoodjs.com/t/welcome-to-the-redwoodjs-community/2416',
  CONTRIBUTORS_MEETUP:
    'https://community.redwoodjs.com/t/contributors-meetups-a-next-step-for-redwoodjs-participation/2470',
  MAKER_HOUR:
    'https://community.redwoodjs.com/t/redwood-makers-hour-stand-up/1971',
  STARTUP_CLUB:
    'https://docs.google.com/forms/d/e/1FAIpQLSeuoGpGJ8gN_eYRY9hU6u2Apw0RB5UV6eYASp6ujsSv_aLaMg/viewform',
  CODE_OF_CONDUCT:
    'https://github.com/redwoodjs/redwood/blob/main/CODE_OF_CONDUCT.md',
  RELEASES:
    'https://community.redwoodjs.com/c/announcements/releases-and-upgrade-guides/18',
  DOCS: 'https://redwoodjs.com/docs/introduction',
  CONFERENCE: 'https://redwoodjsconf.com/',
  STARTUP_FUND: 'http://redwoodstartupfund.com/',
  GITHUB: 'https://github.com/redwoodjs/redwood',
  DISCORD: 'https://discord.com/invite/redwoodjs',
  DISCOURSE: 'https://community.redwoodjs.com/',
  YOUTUBE: 'https://www.youtube.com/redwoodjs',
  X: 'https://twitter.com/redwoodjs',
})
