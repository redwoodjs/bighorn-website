...
This is not technically a breaking change to your code but we have changed how we structure the database file by default and strongly recommend you update your database file to match this new expectation. It's a simple change. We instead expect you to create a distince variable for the Prisma client and export that later as `db`. This is in preparation for future changes that will introduce some awesome features. Here's what the typical diff would look like:
